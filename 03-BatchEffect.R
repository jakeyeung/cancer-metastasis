...
gDes$Batch <- gDes$RunDate
levels(gDes$Batch) <- c(1,2,3,4)
colnames(gDes)
gDesign <- gDes[,c(1,3,11, 5,6,7,8,9)]
setwd("~/Desktop/Grad/Stat540/Project")
write.table(gDesign, "DesignTableBatch.txt", row.names =T)

write.table(gExpDat, "GeneExpData.txt", row.names=TRUE, col.names=T)
source("ComBat.R")
ComBat("GeneExpData.txt", "DesignTableBatch.txt")